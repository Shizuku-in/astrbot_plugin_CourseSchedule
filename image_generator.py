...
class ImageGenerator:
    ...
    async def generate_schedule_image(self, courses: List[Dict], date_type: str = "today") -> str:
        ...
        draw.text(
            (c.GS_PADDING, height - c.GS_PADDING - 55),
            footer_text,
            font=self.font_sub,
            fill=c.US_SUBTITLE_COLOR,
        )
        draw.text(
            (c.GS_PADDING, height - c.GS_PADDING - 25),
            "Generated by Emi",
            font=self.font_sub,
            fill=c.US_SUBTITLE_COLOR,
        )
    ...
    async def generate_user_schedule_image(
        ...
        """为单个用户生成课程表图片"""
        content_inset = 40
        content_vertical_inset = 24
        footer_height = 80
        height = (
            c.US_PADDING * 2
            + content_vertical_inset * 2
            + 100
            + len(courses) * c.US_ROW_HEIGHT
            + footer_height
        )
        image = Image.new("RGB", (c.US_WIDTH, height), c.US_BG_COLOR)
        draw = ImageDraw.Draw(image)

        # 通过图形绘制粗体角标，避免直接使用文字加粗后的变形
        bracket_color = "#26A69A"
        ...
        draw.rectangle(
            [
                bracket_margin,
                bracket_margin,
                bracket_margin + bracket_thickness,
                bracket_margin + bracket_size,
            ],
            fill=bracket_color,
        )
        draw.rectangle(
        ...
        # 右下角」
        right_x = c.US_WIDTH - bracket_margin
        bottom_y = height - bracket_margin
        draw.rectangle(
            [
                right_x - bracket_thickness,
                bottom_y - bracket_size,
                right_x,
                bottom_y,
            ],
            fill=bracket_color,
        ...
        draw.rectangle(
            [
                right_x - bracket_size,
                bottom_y - bracket_thickness,
                right_x,
                bottom_y,
            ],
            fill=bracket_color,
        ...
        content_left = c.US_PADDING + content_inset
        content_right = c.US_WIDTH - c.US_PADDING - content_inset
        content_top = c.US_PADDING + content_vertical_inset
        content_bottom = height - c.US_PADDING - content_vertical_inset

        sanitized_nickname = self._sanitize_for_pil(nickname, self.user_font_title)
        title_suffix_text = self._sanitize_for_pil(title_suffix, self.user_font_title)
        nickname_bbox = self.user_font_title.getbbox(sanitized_nickname)
        ...
        draw.text(
            (content_left + nickname_width, content_top),
            title_suffix_text,
            font=self.user_font_title,
            fill=c.US_TITLE_COLOR,
        )

        y_offset = content_top + 100
        ...
        footer_text = f"Time: {datetime.now().strftime('%Y/%m/%d %H:%M:%S')}"
        draw.text(
            (content_left, content_bottom - 55),
            footer_text,
            font=self.user_font_sub,
            fill=c.US_SUBTITLE_COLOR,
        )
        draw.text(
            (content_left, content_bottom - 25),
            "Generated by Emi",
            font=self.user_font_sub,
            fill=c.US_SUBTITLE_COLOR,
        )
